...
const FileDataInput = ({ 
  ...
  return (
    <Card className="glassmorphic shadow-xl border-[hsl(var(--brighter-teal))]/30 bg-[hsl(var(--card-bg-bright-teal-tint))]">
      <CardHeader>
        <CardTitle className="text-2xl text-primary-foreground">Input Your Financial Data</CardTitle>
        <CardDescription className="text-muted-foreground">Upload up to {MAX_FILES} files (CSV, PDF, PNG, JPG) or paste transaction data.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div>
          <Label htmlFor="transactions-file-input" className="text-lg font-medium text-primary-foreground">Upload Files (Max {MAX_FILES})</Label>
          <div className="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-md border-muted-foreground/50 hover:border-[hsl(var(--boogasi-green-val))] transition-colors">
            <div className="space-y-1 text-center">
              <UploadCloud className={cn("mx-auto h-12 w-12 text-muted-foreground icon-neon-green file-upload-icon")} />
              ...
              <p className="text-xs text-muted-foreground">CSV, PDF, PNG, JPG up to 5MB each</p>
            </div>
          </div>
          {selectedFiles.length > 0 && (
            <div className="mt-4 space-y-2">
              <p className="text-sm font-medium text-primary-foreground">Selected Files ({selectedFiles.length}/{MAX_FILES}):</p>
              <ul className="max-h-40 overflow-y-auto space-y-1 rounded-md border border-border p-2 bg-input/50">
                {selectedFiles.map((file, index) => (
                  <li key={index} className="flex items-center justify-between text-sm text-muted-foreground p-1.5 rounded hover:bg-muted/50">
                    <div className="flex items-center truncate">
                      <FileText className={cn("h-4 w-4 mr-2 flex-shrink-0 icon-neon-green selected-file-icon")} />
        ...
        <div>
          <Label htmlFor="manual-transactions" className="text-lg font-medium text-primary-foreground">Manual Transaction Input (CSV Format)</Label>
          <Textarea
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">Format: Date, Description, Category, Amount (one transaction per line).</p>
        </div>
      </CardContent>
    </Card>
  );
